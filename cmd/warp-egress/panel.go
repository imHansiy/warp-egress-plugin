...
async function loadAll(){
  showError('');
  var result=await Promise.all([api('/status'),api('/rules'),api('/auto'),api('/quality')]);
  state.status=result[0];state.profiles=(state.status&&state.status.profiles)||[];state.rules=normalizeRules(result[1]);state.savedRules=clone(state.rules);state.auto=result[2]||state.auto;state.quality=result[3]||null;state.connected=true;state.page.profiles=1;renderAll();
}
async function loadAuth(){var result=await api('/auth-files');state.files=(result&&result.files)||[];state.authLoaded=true;renderAuth()}
function renderAll(){updateConnection();renderCurrent();renderProfiles();renderSummary();renderRules();renderAuto();renderQuality();renderProvision()}
...
async function saveExtras(){setBusy(el('saveExtrasButton'),true,'保存中');try{state.quality=await api('/quality/save','POST',collectQuality());renderQuality();overlay('extrasOverlay',false);toast('拓展功能已保存','质量守护设置已生效')}catch(e){toast('保存失败',e.message,'error')}finally{setBusy(el('saveExtrasButton'),false)}}
