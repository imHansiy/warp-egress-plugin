...
function renderProfiles(){
  ...
  body.innerHTML=state.profiles.map(function(p){
    ...
    return '<tr><td class="name-cell"><strong>'+esc(p.name)+(current?' <span class="badge blue">当前</span>':'')+'</strong><span>'+esc(mode)+'</span></td><td>'+healthBadge(p)+'</td><td class="mono">'+esc(p.exit_ip_v4||(p.exit_ip&&p.exit_ip.indexOf(':')<0?p.exit_ip:'')||'-')+(p.exit_ip_v6||(p.exit_ip&&p.exit_ip.indexOf(':')>=0?p.exit_ip:'')?'<span class="muted"> / '+esc(p.exit_ip_v6||(p.exit_ip&&p.exit_ip.indexOf(':')>=0?p.exit_ip:''))+'</span>':'')+(duplicate?' <span class="badge amber">重复</span>':'')+'</td></td><td><span>'+esc(p.colo||'-')+'</span><span class="muted"> · '+(p.latency_ms?p.latency_ms+' ms':'-')+'</span></td><td class="mono muted">'+esc(p.proxy_url||'-')+'</td><td><div class="actions">'+(current?'':'<button class="btn soft small" data-action="set-global" data-id="'+esc(p.id)+'">设为全局</button>')+'<button class="btn secondary small" data-action="profile-check" data-id="'+esc(p.id)+'">检测</button><div class="menu"><button class="btn secondary small" data-action="toggle-menu">更多</button><div class="menu-pop">'+(p.mode==='managed'?'<button data-action="profile-'+(p.running?'stop':'start')+'" data-id="'+esc(p.id)+'">'+(p.running?'停止':'启动')+'</button><button data-action="profile-recreate" data-id="'+esc(p.id)+'">重新注册</button>':'')+'<button class="danger-text" data-action="profile-delete" data-id="'+esc(p.id)+'">删除</button></div></div></div></td></tr>'
  }).join('')
}
