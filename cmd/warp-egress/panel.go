  ...
  <section class="current">
    <div class="current-main">
      ...
      <div class="current-meta">
        <span id="currentLocation">节点 -</span>
  ...
  <section class="section">
    ...
    <div class="table-wrap">
      ...
      <div id="profilesTableWrap" class="table-scroll">
        <table>
          <thead><tr><th>名称</th><th>状态</th><th>公网 IP</th><th>节点 / 延迟</th><th>本地代理</th><th style="width:190px">操作</th></tr></thead>
...
function renderCurrent(){
  ...
  el('currentName').textContent=p?p.name:'未选择';el('currentIP').textContent=p?(p.exit_ip||'等待检测'):'-';el('currentHealth').outerHTML=healthBadge(p).replace('<span','<span id="currentHealth"');el('currentLocation').textContent='节点 '+(p&&p.colo?p.colo:'-');el('currentLatency').textContent='延迟 '+(p&&p.latency_ms?p.latency_ms+' ms':'-');el('relayURL').textContent=s.global_relay_url||'-';
...
function renderProfiles(){
  ...
  body.innerHTML=state.profiles.map(function(p){
    ...
    return '<tr><td class="name-cell"><strong>'+esc(p.name)+(current?' <span class="badge blue">当前</span>':'')+'</strong><span>'+esc(mode)+'</span></td><td>'+healthBadge(p)+'</td><td class="mono">'+esc(p.exit_ip||'-')+(duplicate?' <span class="badge amber">重复</span>':'')+'</td><td><span>'+esc(p.colo||'-')+'</span><span class="muted"> · '+(p.latency_ms?p.latency_ms+' ms':'-')+'</span></td><td class="mono muted">'+esc(p.proxy_url||'-')+'</td><td><div class="actions">'+(current?'':'<button class="btn soft small" data-action="set-global" data-id="'+esc(p.id)+'">设为全局</button>')+'<button class="btn secondary small" data-action="profile-check" data-id="'+esc(p.id)+'">检测</button><div class="menu"><button class="btn secondary small" data-action="toggle-menu">更多</button><div class="menu-pop">'+(p.mode==='managed'?'<button data-action="profile-'+(p.running?'stop':'start')+'" data-id="'+esc(p.id)+'">'+(p.running?'停止':'启动')+'</button><button data-action="profile-recreate" data-id="'+esc(p.id)+'">重新注册</button>':'')+'<button class="danger-text" data-action="profile-delete" data-id="'+esc(p.id)+'">删除</button></div></div></div></td></tr>'
  }).join('')
}
