...
<div id="extrasOverlay" class="overlay">
  <div class="modal wide">
    ...
    <div id="extrasQuality" class="modal-body auto-grid">
      <div class="setting"><div class="setting-row"><div><strong>xAI 降智守护</strong><p>仅针对 xAI / Grok 模型输出（共享出口被打穿时输出 token/s 异常飙升）。所有 xAI 认证文件保持跟随 CPA 全局代理，本功能不会批量写入认证文件 proxy_url；当前全局出口被判定降智时，只切换到近期实测健康的备用出口。其他 provider、手工分流与规则功能不受影响。</p></div><label class="toggle"><input id="qualityEnabled" type="checkbox"><span></span></label></div>
...
function collectQuality(){
  var probe={enabled:el('qualityProbe').checked,model:el('qualityProbeModel').value.trim(),interval_minutes:Number(el('qualityProbeInterval').value||0)};
