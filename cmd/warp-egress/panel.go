...
<div id="extrasOverlay" class="overlay">
  <div class="modal wide">
    ...
    <div class="tabs"><button class="tab active" data-extra-tab="quality">xAI 出口守护扩展</button></div>
    <div id="extrasQuality" class="modal-body auto-grid">
      <div class="setting"><div class="setting-row"><div><strong>xAI 专用出口路由</strong><p>独立模式下，即使普通全局代理选择“不使用代理”，xAI 目标域名仍只走这里的健康出口；没有健康出口时阻断 xAI，不会回退服务器直连。CPA 的 proxy-url 必须继续指向本插件本地中继。</p></div></div>
        <div class="field"><label>路由模式</label><select id="qualityRouteMode" class="select"><option value="independent">独立 xAI 出口（推荐）</option><option value="follow_global">跟随普通全局出口</option><option value="direct">xAI 直连</option></select></div>
        <div class="field"><label>当前 xAI 活动出口</label><select id="qualityActiveProfile" class="select"></select><div class="route-note">自动切换只更新这里，不改普通全局代理。</div></div>
        <div class="field"><label>xAI 目标域名（每行一个，支持 *.example.com）</label><textarea id="qualityRouteHosts" class="input mono" rows="3" placeholder="cli-chat-proxy.grok.com&#10;api.x.ai"></textarea><div class="route-note">自定义 base_url 必须把其域名加入这里；相同网关域名同时承载其他 provider 时无法按类型区分。</div></div></div>
      <div class="setting"><div class="setting-row"><div><strong>启用 xAI 出口守护扩展</strong><p>这是可选扩展，不属于插件核心代理能力。关闭后不识别 xAI 域名、不主动探测、不切换 xAI 出口，本地中继完全恢复普通全局代理行为；认证文件始终不批量写 proxy_url。</p></div><label class="toggle"><input id="qualityEnabled" type="checkbox"><span></span></label></div>
        <div class="field"><label>软阈值（输出 token/秒）</label><input id="qualitySoftTPS" class="input" type="number" min="1" step="10"><div class="route-note">连续命中后复测并隔离，默认 500。</div></div>
        <div class="field"><label>硬阈值（输出 token/秒）</label><input id="qualityHardTPS" class="input" type="number" min="1" step="10"><div class="route-note">单次命中立即隔离，不等待软信号累计，默认 1000。</div></div>
        <div class="field"><label>软阈值连续次数</label><input id="qualityConsecutive" class="input" type="number" min="1" step="1"></div>
        <div class="field"><label>探测错误连续次数</label><input id="qualityConsecutiveErrors" class="input" type="number" min="1" step="1"><div class="route-note">账号过期、额度耗尽和暂无可用账号不计入。</div></div>
        <div class="setting-row"><div><strong>thinking 缺失判定</strong><p>足够长的响应缺少 thinking_content / reasoning_content 时视为降智信号。</p></div><label class="toggle"><input id="qualityThinkingGuard" type="checkbox"><span></span></label></div>
        <div class="field"><label>连续缺少 thinking 次数</label><input id="qualityMissingThinking" class="input" type="number" min="1" step="1"></div>
        <div class="setting-row"><div><strong>thinking 异常交叉复测</strong><p>真实请求缺 thinking 达到阈值后，用另一个健康探针账号复测同一出口再隔离。</p></div><label class="toggle"><input id="qualityThinkingCross" type="checkbox"><span></span></label></div>
        <div class="setting-row"><div><strong>TPS 异常交叉复测</strong><p>TPS 连续异常达到阈值后主动复测一次，减少短响应和偶发抖动误判。</p></div><label class="toggle"><input id="qualitySoftCross" type="checkbox"><span></span></label></div>
        <div class="field"><label>连续健康次数恢复</label><input id="qualityRecovery" class="input" type="number" min="1" step="1"></div></div>
      <div class="setting"><div class="setting-row"><div><strong>自动补充出口</strong><p>近期 xAI 实测健康的托管出口不足时自动注册新 WARP 出口（经现有连通出口注册，带冷却防限流）。</p></div><label class="toggle"><input id="qualityProvision" type="checkbox"><span></span></label></div>
        <div class="field"><label>最低健康出口数</label><input id="qualityMinHealthy" class="input" type="number" min="1" step="1"><div class="route-note">同时用于自动补充与隔离保护；隔离后低于该数量时抑制隔离，避免全军覆没。</div></div>
      ...
      <div class="setting"><div class="setting-row"><div><strong>备用出口主动探测（xAI）</strong><p>后台每轮只错峰检测一个出口，只复用 CPA 中未禁用、未冷却且当前可用的少量 xAI 账号。探测不会绑定账号代理；结果用于准备健康备用出口，避免数千账号时形成扫描和写入风暴。</p></div><label class="toggle"><input id="qualityProbe" type="checkbox"><span></span></label></div>
        <div class="field"><label>xAI 模型（如 grok-4.6）</label><input id="qualityProbeModel" class="input" placeholder="grok-4.6"></div>
        <div class="field"><label>同一出口最短复检间隔（分钟）</label><input id="qualityProbeInterval" class="input" type="number" min="1" step="1"></div></div>
    </div>
...
function qualityBadge(p){
  ...
  if(p.degraded){var label=p.quality_classification==='hard'?'硬隔离':(p.quality_classification==='error'?'错误隔离':'降智');return '<span class="badge red" title="'+esc(p.degraded_reason||'')+'">'+label+'</span>'}
  if(p.quality_classification==='suppressed')return '<span class="badge amber" title="'+esc(p.quality_error||'最低健康出口保护已生效')+'">已抑制</span>';
  if(p.quality_classification==='hard')return '<span class="badge red" title="单次 TPS 命中硬阈值">硬异常</span>';
  if(p.quality_classification==='degraded')return '<span class="badge red" title="最近探测 '+(p.quality_tps!=null?Number(p.quality_tps).toFixed(1)+' token/s':'异常')+'">探测异常</span>';
  if(p.quality_classification==='error')return '<span class="badge amber" title="'+esc(p.quality_error||'最近一次主动探测未完成')+'">探测失败</span>';
  if(p.quality_classification==='healthy')return '<span class="badge blue" title="最近 '+(p.quality_tps!=null?Number(p.quality_tps).toFixed(1)+' token/s':'健康')+'">健康</span>';
...
function renderQuality(){
  ...
  var route=q.route||{};
  el('qualityRouteMode').value=route.mode||'independent';
  var active=route.active_profile_id||'';
  el('qualityActiveProfile').innerHTML='<option value="">自动选择健康出口</option>'+state.profiles.map(function(p){return '<option value="'+esc(p.id)+'"'+(p.id===active?' selected':'')+'>'+esc(p.name)+(p.healthy&&!p.degraded?'':'（当前不可用）')+'</option>'}).join('');
  el('qualityActiveProfile').disabled=el('qualityRouteMode').value!=='independent';
  el('qualityRouteHosts').value=(route.hosts||['cli-chat-proxy.grok.com','api.x.ai']).join('\n');
  el('qualityEnabled').checked=!!q.enabled;
  el('qualitySoftTPS').value=q.soft_tps!=null?q.soft_tps:500;
  el('qualityHardTPS').value=q.hard_tps!=null?q.hard_tps:1000;
  el('qualityConsecutive').value=q.consecutive_degraded!=null?q.consecutive_degraded:3;
  el('qualityConsecutiveErrors').value=q.consecutive_errors!=null?q.consecutive_errors:3;
  el('qualityThinkingGuard').checked=q.thinking_guard!==false;
  el('qualityMissingThinking').value=q.consecutive_missing_thinking||1;
  el('qualityThinkingCross').checked=q.thinking_cross_verify!==false;
  el('qualitySoftCross').checked=q.soft_cross_verify!==false;
  ...
  el('qualityProbeModel').value=(q.probe&&q.probe.model)||'grok-4.6';
  el('qualityProbeInterval').value=(q.probe&&q.probe.interval_minutes)||15;
}
function collectQuality(){
  var current=(state.quality&&state.quality.quality)||{};var currentProbe=current.probe||{};
  var probe={enabled:el('qualityProbe').checked,model:el('qualityProbeModel').value.trim(),max_tokens:currentProbe.max_tokens||128,timeout_seconds:currentProbe.timeout_seconds||60,interval_minutes:Number(el('qualityProbeInterval').value||0)};
  var hosts=el('qualityRouteHosts').value.split(/[\n,]+/).map(function(v){return v.trim()}).filter(Boolean);
  var route={mode:el('qualityRouteMode').value,active_profile_id:el('qualityActiveProfile').value,hosts:hosts};
  return {policy_schema:4,enabled:el('qualityEnabled').checked,soft_tps:Number(el('qualitySoftTPS').value||0),hard_tps:Number(el('qualityHardTPS').value||0),consecutive_degraded:Number(el('qualityConsecutive').value||0),consecutive_errors:Number(el('qualityConsecutiveErrors').value||0),thinking_guard:el('qualityThinkingGuard').checked,consecutive_missing_thinking:Number(el('qualityMissingThinking').value||0),thinking_cross_verify:el('qualityThinkingCross').checked,soft_cross_verify:el('qualitySoftCross').checked,recovery_observations:Number(el('qualityRecovery').value||0),min_generation_ms:current.min_generation_ms||1000,min_output_tokens:current.min_output_tokens!=null?current.min_output_tokens:32,auto_provision:el('qualityProvision').checked,min_healthy:Number(el('qualityMinHealthy').value||0),auto_prune:el('qualityPrune').checked,max_profiles:Number(el('qualityMaxProfiles').value||0),provision_cooldown_minutes:current.provision_cooldown_minutes||15,probe:probe,route:route};
}
...
function demoSeed(){state.status={version:'0.3.0',global_relay_url:'socks5://127.0.0.1:40000',required_host_proxy_url:'socks5://127.0.0.1:40000',global_profile_id:'sin-main',global_relay_running:true,duplicate_exit_ips:{},profiles:[{id:'sin-main',name:'新加坡主出口',mode:'managed',proxy_url:'socks5://127.0.0.1:41000',running:true,healthy:true,quality_classification:'healthy',quality_tps:74,quality_checked_at:new Date().toISOString(),exit_ip:'2a09:bac1:6540:8::',exit_ip_v4:'104.28.210.10',exit_ip_v6:'2a09:bac1:6540:8::',colo:'SIN',latency_ms:28,last_checked:new Date().toISOString()},{id:'nrt-backup',name:'东京备用出口',mode:'managed',proxy_url:'socks5://127.0.0.1:41001',running:true,healthy:true,quality_classification:'healthy',quality_tps:69,quality_checked_at:new Date().toISOString(),exit_ip:'104.28.210.12',exit_ip_v4:'104.28.210.12',exit_ip_v6:'2a09:bac0:1:2::',colo:'NRT',latency_ms:61,last_checked:new Date().toISOString()}]};state.status.global_profile=state.status.profiles[0];state.profiles=state.status.profiles;state.rules={global_profile_id:'sin-main',type_rules:[{key:'codex',profile_id:'nrt-backup',enabled:true}],regex_rules:[],exact_rules:{}};state.savedRules=clone(state.rules);state.auto={enabled:false,failover_enabled:true,rotate_interval_seconds:0,require_different_ip:true};state.quality={quality:{policy_schema:4,enabled:true,soft_tps:500,hard_tps:1000,consecutive_degraded:3,consecutive_errors:3,thinking_guard:true,consecutive_missing_thinking:1,thinking_cross_verify:true,soft_cross_verify:true,recovery_observations:2,min_healthy:2,max_profiles:8,auto_provision:true,auto_prune:true,route:{mode:'independent',active_profile_id:'sin-main',hosts:['cli-chat-proxy.grok.com','api.x.ai']},probe:{enabled:true,model:'grok-4.6',interval_minutes:15}}};state.settings={cleanup_unhealthy_enabled:false,cleanup_unhealthy_minutes:10,system_proxy:{enabled:false,port:40001}};state.connected=true;renderAll()}
