...
async function configExport(){try{var r=await api('/config/export','GET');var decoded=String(r.state_json||'').replace(/&#34;/g,'"').replace(/&#39;/g,"'").replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&amp;/g,'&');var compact=JSON.stringify(JSON.parse(decoded));var snippet="state-json: '"+compact.replace(/'/g,"''")+"'";await copy(snippet);toast('已复制配置段','粘贴到 config.yaml 的 plugins.configs.warp-egress 下（state-json 字段），重启后生效；配置了 CPA 外部数据库时随配置体系统一管理')}catch(e){toast('导出失败',e.message,'error')}}
