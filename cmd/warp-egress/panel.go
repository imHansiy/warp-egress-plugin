  ...
  <div class="modal wide">
    ...
    <div id="extrasQuality" class="modal-body auto-grid">
      ...
      <div class="setting"><div class="setting-row"><div><strong>备用出口主动探测（xAI）</strong><p>后台每轮只错峰检测一个出口，只复用 CPA 中未禁用、未冷却且当前可用的少量 xAI 账号。探测不会绑定账号代理；结果用于准备健康备用出口，避免数千账号时形成扫描和写入风暴。</p></div><label class="toggle"><input id="qualityProbe" type="checkbox"><span></span></label></div>
        <div class="field"><label>xAI 模型（如 grok-4）</label><input id="qualityProbeModel" class="input" placeholder="grok-4"></div>
...
function renderQuality(){
  ...
  el('qualityProbeModel').value=(q.probe&&q.probe.model)||'grok-4';
