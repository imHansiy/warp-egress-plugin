...
| xAI 降智守护 | xAI 账号统一走全局出口；后台错峰探测备用出口，降智时只切换到近期实测健康出口 |
...
全局切换只改变固定中继的后端，无需重启 CLIProxyAPI。认证文件级规则通过 `host.auth.get` / `host.auth.save` 回调修改认证 JSON 的 `proxy_url` 字段——与 CPA 自带「认证文件代理」是同一个字段，两处完全同步。xAI 降智守护本身不会批量修改认证文件；未手工分流的 xAI 文件保持无 `proxy_url` 并继承全局中继。
...
「拓展功能 → xAI 降智守护」只改变 xAI / Grok 的出口质量维护逻辑，不接管其他 provider，也不改变手工认证文件分流功能：

- xAI 认证文件默认继承 CPA 全局代理，守护不会扫描并批量写入 `proxy_url`。
- 真实 xAI 请求继续被动统计当前全局出口的输出 TPS；达到连续异常阈值后标记降智。
- 主动探测每轮只检查一个最久未测的出口，并保证同一时刻最多一个探测请求；筛出的少量探针账号在内存复用 5 分钟，避免每个出口都重新拉取数千账号。
- 探针账号只从 CPA 当前未禁用、未标记不可用、未处于冷却期的 xAI 账号中选取；账号的 401、403、402 或 429 会换下一个账号，不会误判为出口降智。
- 当前全局出口降智时，近期实测健康的备用出口可直接接管；结论过期的候选先探测，确认健康后才切换。
