...
- 真实 xAI 请求按实际活动出口统计输出 TPS，并识别 `thinking_content`、`reasoning_content` 或 `reasoning_tokens`。缺 thinking 与 TPS 异常分别连续计数。
- 被动 thinking/TPS 异常达到阈值后默认主动交叉复测同一出口；同一出口最多一个复测任务，避免请求高峰形成 Token 消耗风暴。
